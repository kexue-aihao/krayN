...
public static class ConfigHandler
{
    ...
    public static async Task<int> AddKrayServer(Config config, ProfileItem profileItem, bool toFile = true)
    {
        ...
        profileItem.Network = profileItem.Network.TrimEx().IsNullOrEmpty() ? "tcp" : profileItem.Network.TrimEx();
        if (profileItem.Address.IsNullOrEmpty()
            ...
            || profileItem.PublicKey.IsNullOrEmpty())
        {
            return -1;
        }
    ...
    private static async Task<int> AddBatchServers4Custom(Config config, string strData, string subid, bool isSub, IEnumerable<ProfileItem>? oldProfiles = null)
    {
        ...
        if (lstProfiles is null || lstProfiles.Count <= 0)
        {
            lstProfiles = KrayFmt.ResolveJsonSubscription(strData, subRemarks);
            isKraySubscription = lstProfiles is { Count: > 0 };
        }
