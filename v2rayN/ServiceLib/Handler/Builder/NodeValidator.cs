...
public class NodeValidator
{
    ...
    private static void ValidateNodeAndCoreSupport(ProfileItem item, ECoreType coreType, ValidationContext v)
    {
        ...
        else if (coreType is ECoreType.kray)
        {
            if (!Global.KraySupportConfigType.Contains(item.ConfigType))
            {
                v.Error(string.Format(ResUI.MsgCoreNotSupportProtocol, nameof(ECoreType.kray), item.ConfigType));
            }
        }
