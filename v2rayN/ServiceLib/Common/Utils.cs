...
public class Utils
{
    ...
    public static bool IsPackagedInstall()
    {
        try
        {
            ...
            if (p.StartsWith("/opt/v2rayN", StringComparison.OrdinalIgnoreCase) || p.StartsWith("/opt/krayN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }

            if (p.StartsWith("/usr/lib/v2rayN", StringComparison.OrdinalIgnoreCase) || p.StartsWith("/usr/lib/krayN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }

            if (p.StartsWith("/usr/share/v2rayN", StringComparison.OrdinalIgnoreCase) || p.StartsWith("/usr/share/krayN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        }
